...
for i = 1:length(networks)
    ...
    temp = ['TEMP/' networks{i} '_linkrem10_stacking_topological_CH_SPM_SBM_linkpred.TEMP'];
    outfile = ['results/' networks{i} '_linkrem10_stacking_topological_CH_SPM_SBM_linkpred.mat'];
    ...
    load(['../SBM/scores/' networks{i} '_linkrem10_SBM_DC_scores.mat'], 'scores');
    SBM_scores = scores; clear scores;
    load(['../SBM/scores/' networks{i} '_linkrem10_linkrem20noconn_SBM_DC_scores.mat'], 'scores');
    SBM_scores_train = scores; clear scores;

    measures = cell(length(matrices),1);
    parfor j = 1:length(matrices)
        % link prediction
        fileprefix = sprintf('TEMP/%d_%d', i, j);
        scores = stacking_topological_CH_SPM_SBM_wrapper(matrices{j}, matrices_train{j}, edges_train{j}, labels_train{j}, fileprefix, SBM_scores{j}, SBM_scores_train{j});
